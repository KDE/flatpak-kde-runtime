...
all: $(REPO)/config org.kde.Sdk.json
	flatpak-builder --rebuild-on-sdk-change --arch=$(ARCH) --force-clean --require-changes --ccache --repo=$(REPO) --subject="build of org.kde.Sdk, `date`" ${EXPORT_ARGS} $(TMP) org.kde.Sdk.json
